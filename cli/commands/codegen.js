...
const generatorMap = {
  ...
  ts: require('../../ts_generator/lib/generator')
...
function generateCode(options) {
  ...
  const generator = langSubVersionMap[options.lang]
  ...
  if (config.exec === true && !helper.isExecutable(ast)) {
    ...
    console.log(
      chalk.red(
        `There is no static main function in dara, exec option can't be used!`
      )
    );
    console.log();
    process.exit(-1);
...
class CodegenCommand extends Command {
  ...
  async exec(args, options) {
    ...
    if (pkg.mode === 'interface') {
      console.log();
      console.log(
        chalk.red(`The package is interface mode, the SDK can not be generated`)
      );
      console.log();
      process.exit(-1);
    }
