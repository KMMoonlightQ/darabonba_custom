...
const Tea = require('../../index');
const Annotation = require('@darabonba/annotation-parser');
const { PKG_FILE, README_FILE } = require('./constants');
const REQUIRED_RELEASE_LANGUAGE = [
  'java',
  'ts',
  'go',
  'php',
...
class AstFormatter {
  ...
  getAstData() {
    ...
    const { apiObj, functionObj, initData, isInterface } =
      this._parseModuleBody(moduleBodyNodes);
    ...
    const moduleAnno = this._formatAnnotation(
      _get(this.checkData, 'ast.annotation.value', '')
    );
    this.astData = {
      api: apiObj,
    ...
    moduleBodyNodes.forEach((node) => {
      switch (node.type) {
        case 'api':
          ...
          break;
        case 'function':
          functionObj[node.functionName.lexeme] = this._parseModuleNode(
            node,
            'function'
          );
          isInterface = functionObj[node.functionName.lexeme].isInterface
            ? true
            : isInterface;
          break;
        case 'init':
  ...
  _parseModuleNode(data, type) {
    ...
    if (data.annotation && data.annotation.value) {
      ...
      result.annotation.params &&
        result.annotation.params.forEach((item) => {
  ...
  _formatNodeParamType(param, paramAnnotation) {
    let item = {
      ...
      defaultValue:
        param.defaultValue &&
        (param.defaultValue.lexeme || param.defaultValue.string)
    };
    ...
    if (item.paramType === 'array') {
      item.paramItemType =
        param.paramType.subType && param.paramType.subType.lexeme;
    }
  ...
  getScore() {
    ...
    this._countApiScore(
      Object.values(_get(this.astData, 'function', {})),
      'function'
    );
    ...
    countResult.annotation = Math.ceil(
      (this.requiredScore.filled / this.requiredScore.total) * 70 +
    ...
    if (this.pkg && this.pkg.releases) {
      ...
      countResult.release = Math.ceil(
        (releaseCount * 100) / REQUIRED_RELEASE_LANGUAGE.length
      );
    }
    countResult.total = Math.ceil(
      countResult.annotation * 0.7 +
        countResult.readme * 0.2 +
        countResult.release * 0.1
    );
    return countResult;
  }
  ...
  _formatModelNode(model) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (item.fieldType === 'array') {
        item.fieldItemType =
          nodes[i].fieldValue.fieldItemType.lexeme ||
          nodes[i].fieldValue.fieldItemType.type;
      ...
      if (typeof item.fieldType !== 'string') {
        if (item.fieldType.lexeme) {
          ...
          item.fieldType.type === 'moduleModel' &&
          item.fieldType.path
        ) {
