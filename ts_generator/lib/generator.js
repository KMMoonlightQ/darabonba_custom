...
class Visitor {
  ...
  constructor(option = {}) {
    this.config = {
      ...
      packageName: '',
      clientPath:
        option.testFile === true ? 'test/client.spec.ts' : 'src/client.ts',
      ...option
    };
    this.output = '';
  ...
  visitModule(ast, level) {
    ...
    const subModels = Object.keys(this.predefined)
      .filter((key) => {
        return (
          !key.startsWith('$') &&
          this.predefined[key].type === 'model' &&
          key.indexOf('.') !== -1
        );
      })
      .map((key) => {
...
Client.main(process.argv.slice(2));`);
  ...
  visitAnnotation(annotation, level) {
    ...
    let comments = DSL.comment.getFrontComments(
      this.comments,
      annotation.index
    );
  ...
  eachImport(imports, usedModels, level) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        const pkgPath = fs.existsSync(path.join(targetPath, 'Teafile'))
  ...
  visitRuntimeBefore(ast, level) {
    ...
    this.emit(
      `while (${CORE}.allowRetry(_runtime['retry'], _retryTimes, _now)) {\n`,
      level
    );
    this.emit('if (_retryTimes > 0) {\n', level + 1);
    this.emit(
      `let _backoffTime = ${CORE}.getBackoffTime(_runtime['backoff'], _retryTimes);\n`,
      level + 2
    );
  ...
  visitFieldType(value, level, modelName, fieldName) {
    if (value.type === 'modelBody') {
      ...
      this.emit(
        `$${moduleId.lexeme}.${_subModelName(
          models.map((item) => item.lexeme).join('.')
        )}`
      );
  ...
  visitFieldTypeString(value, level, modelName, fieldName) {
    if (value.type === 'modelBody') {
      ...
      this.emit(
        `{ 'type': 'map', 'keyType': '${value.keyType.lexeme}', 'valueType': `
      );
  ...
  visitModelBody(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.visitFieldType(
        ...
        level,
        modelName,
        _name(node.fieldName)
      );
      this.emit(';\n');
    }
    if (node) {
      //find the last node's back comment
      let comments = DSL.comment.getBetweenComments(
        this.comments,
        node.tokenRange[0],
        ast.tokenRange[1]
      );
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (nameAttr) {
        this.emit(
          `${_name(node.fieldName)}: '${_string(nameAttr.attrValue)}',\n`,
          level + 2
        );
      } else {
        this.emit(
          `${_name(node.fieldName)}: '${_name(node.fieldName)}',\n`,
          level + 2
        );
      }
    }
  ...
  visitArray(ast, level) {
    ...
    let arrayComments = DSL.comment.getBetweenComments(
      this.comments,
      ast.tokenRange[0],
      ast.tokenRange[1]
    );
  ...
  visitIf(ast, level) {
    ...
    if (ast.elseStmts) {
      ...
      if (ast.elseStmts.stmts.length === 0) {
        const comments = DSL.comment.getBetweenComments(
          this.comments,
          ast.elseStmts.tokenRange[0],
          ast.elseStmts.tokenRange[1]
        );
        this.visitComments(comments, level + 1);
      }
      this.emit('}', level);
  ...
  visitStmts(ast, level) {
    ...
    if (node) {
      //find the last node's back comment
      let comments = DSL.comment.getBackComments(
        this.comments,
        node.tokenRange[1]
      );
      this.visitComments(comments, level);
  ...
  eachAPI(ast, level) {
    ...
    this.emit(
      `let ${RESPONSE} = await ${CORE}.doAction(${REQUEST}`,
      baseLevel + 1
    );
